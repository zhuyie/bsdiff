...
TEST_F(BSDiffApiTest, InvalidArgumentChecks) {
  ...
  // Test NULL ctx
  int ret = bsdiff(nullptr, &old_stream, &new_stream, &packer);
  EXPECT_EQ(ret, BSDIFF_INVALID_ARG);

  // Test NULL oldfile
  ret = bsdiff(&ctx, nullptr, &new_stream, &packer);
  EXPECT_EQ(ret, BSDIFF_INVALID_ARG);

  // Test NULL newfile
  ret = bsdiff(&ctx, &old_stream, nullptr, &packer);
  EXPECT_EQ(ret, BSDIFF_INVALID_ARG);

  // Test NULL packer
  ret = bsdiff(&ctx, &old_stream, &new_stream, nullptr);
  EXPECT_EQ(ret, BSDIFF_INVALID_ARG);
}
...
class ZstdBSDiffApiTest : public ::testing::Test {
protected:
  void SetUp() override {
    // Setup dummy streams for valid stream tests
    const char *old_data = "dummy old data";
    ...
    // Setup a real zstd patch packer using the memory stream
    bsdiff_open_memory_stream(BSDIFF_MODE_WRITE, nullptr, 1024, &packer_stream);

    bsdiff_open_zstd_patch_packer(BSDIFF_MODE_WRITE, &packer_stream, &packer);
  }
...
// Test NULL arguments on core bsdiff API using zstd
TEST_F(ZstdBSDiffApiTest, InvalidArgumentChecks) {
  struct bsdiff_ctx ctx;
  memset(&ctx, 0, sizeof(ctx));
...
TEST_F(ZstdBSDiffApiTest, GeneratePatch) {
  // Test generating a valid patch stream between old_stream and new_stream
  struct bsdiff_ctx ctx;
  memset(&ctx, 0, sizeof(ctx));
  int ret = bsdiff(&ctx, &old_stream, &new_stream, &packer);
